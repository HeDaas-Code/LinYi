...
参考 ai-town `memory.ts`：

```python
@dataclass
class OCMemory:
    description: str       # 第一人称摘要
    importance: float      # 0-9，由 LLM 打分
    embedding: list[float]
    last_access: float
    memory_type: str       # conversation / reflection / observation

class OCMemorySystem:
    def remember_conversation(self, oc: OCAgent, conversation: OCConversation):
        summary = llm_summarize_from_perspective(oc, conversation)
        importance = llm_rate_importance(summary)
        embedding = embed(summary)
        self.store(oc, summary, importance, embedding, type='conversation')
        self.reflect_on_memories(oc)

    def search(self, oc: OCAgent, query_embedding, n=3):
        candidates = vector_search(oc, query_embedding, n * 10)
        # 按 relevance + recency + importance 综合排序
        return rank_by_relevance_recency_importance(candidates, n)

    def reflect_on_memories(self, oc: OCAgent):
        recent_memories = self.get_recent(oc, 100)
        new_importance = sum(m.importance for m in recent_memories if m.new)
        if new_importance > REFLECTION_THRESHOLD:
            insights = llm_reflect(oc, recent_memories)
            for insight in insights:
                self.store(oc, insight, ...)
```

### 6.7 RelationshipGraph

- 记录 OC 之间是否共同参与过事件/对话（`participatedTogether`）。
- 为社交模拟提供"冷却期"，避免同一对角色反复对话。
...
### 14.2 跨项目提取的"虚拟生命"核心能力矩阵

将 18 个项目的能力抽象到一张表，**空白格子就是 LinYi 缺什么**：

| 能力 | generative_agents | ai-town | AstrBot | MaiBot | mem0 | Zep | MemGPT | SillyTavern | astrbot_companion | autogen | **LinYi 现状** |
|------|---|---|---|---|---|---|---|---|---|---|---|
...
#### 14.3.1 generative_agents — 记忆流的源头
...
基于 14.2 的能力矩阵和 14.3 的项目提炼，给出 **LinYi 升级为"虚拟生命"的 5 个核心补强方向**：
...
> LinYi 要成为"生活在电脑中、有自己人格和社交的虚拟生命"，核心是 **升级记忆（分层+三因子+反思）、补强节律（两层日程+自我时间线）、引入 OC 自治社交（ai-town 模式）、对齐角色卡规范（SillyTavern 兼容）、治理成本（内部 TokenBudget）**。这五件事全部基于 `references/` 中已下载的 18 个开源项目的能力提炼。
